    ...
    public:
        ...
        void printAll() {
            for(int i = 0; i < nodes.size(); i++) {
                ...
                for(int j = 0; j < nodes[i].getDegree(); j++) {
                    std::cout << "\t" << nodes[i]._paths[j].first->city << "\t" << nodes[i]._paths[j].second._dist << "\n";
                }
            }
        }
        ...
        void assembleFragments(std::vector<std::string>& fragments, std::string& assembly) {
            ...
            while(len != 1)
            {
                ...
                // String of overlapping fragments with maximum overlap
                std::string resStr;   
                ...
                for (int i = 0; i < len; i++) {
                    for (int j = i + 1; j < len; j++) {
                        std::string str;
                        int res = findOverlappingPair(fragments[i], fragments[j], str); 
                ...
                if (max == INT_MIN) {
                    ...
                    // Copy resultant string to left index
                    fragments[leftIdx] = resStr; 
                    // Copy string at last index to right index
                    fragments[rightIdx] = fragments[len]; 
                }
            }
            assembly = fragments[0];
        }
            ...
            for (int count = 0; count < V - 1; count++) {
                ...
                for (int v = 0; v < V; v++) {
                    ...
                    if (!sptSet[v] && _isConnected(u, v) && dist[u].first != INT_MAX && dist[u].first + getEdge(u, v)._dist < dist[v].first) {
                        // std::cout << "if worked\tw = " << nodes[v].city <<"\n";
